...
func (h *Hub) CreateAndBroadcast(senderID uint, input CreateMessageInput) (MessageInfo, error) {
	...
	if input.Type == MessageTypeVideoShare {
		...
		if err := h.svc.DB.Where("id = ? AND status = ?", input.VideoID, "approved").First(&video).Error; err != nil {
...
func ToMessageInfo(message model.ChatMessage) MessageInfo {
	...
	if message.SharedVideoID != nil && message.SharedVideo.ID != 0 {
		info.Video = &SharedVideoInfo{
			...
			Duration: message.SharedVideo.Duration,
			Author:   model.UserInfo{ID: message.SharedVideo.Author.ID, Username: message.SharedVideo.Author.Username, Nickname: message.SharedVideo.Author.Nickname, Avatar: message.SharedVideo.Author.Avatar, Role: message.SharedVideo.Author.Role},
		}
	}
	return info
}
