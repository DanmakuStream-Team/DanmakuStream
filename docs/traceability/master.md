...
> 状态统计：三层图 **24/39**（缺 UC01/06/07/08/11 共 15 张）；三层测试齐备且通过 **4/13**（UC05/09/10/13）；部分通过 1/13（UC04，与 UC13 共享审核链路）；未测 8/13。

| 需求 | 用例 | 系统级图 | 组件级图 | 对象级图 | 主要代码模块 | 单元测试 | API/集成测试 | E2E 测试 | 结果 | 缺口（负责人） |
|---|---|---|---|---|---|---|---|---|---|---|
| REQ01 | UC01 注册/登录/资料维护 | ❌ | ❌ | ❌ | `auth/auth_handler.go`、`user/user_handler.go`、`LoginPage.vue`、`RegisterPage.vue`、`UserProfilePage.vue` | — | — | — | 未测 | 三层图×3；注册重名/密码错误/身份过期等异常测试（B） |
...
| REQ06 | UC06 个人视频资料库管理 | ❌ | ❌ | ❌ | `collection/`、`user/`（历史/进度/稍后再看）、`UserLibraryPage.vue` | — | — | — | 未测 | 三层图×3；服务端历史/进度同步测试（E） |
| REQ07 | UC07 关注关系与内容通知 | ❌ | ❌ | ❌ | `user`（关注/分组/特别关注/屏蔽）、`notification/` | — | — | — | 未测 | 三层图×3；分组、特别关注、屏蔽规则测试（B） |
| REQ08 | UC08 创作者会员订阅 | ❌ | ❌ | ❌ | `membership/`（方案/订单/演示支付/订阅）、`SubscriptionPage.vue` | — | — | — | 未测 | 三层图×3；会员方案、订单状态机、订阅权限测试（B） |
...
| REQ11 | UC11 用户私信与媒体分享 | ❌ | ❌ | ❌ | `message/`、`ChatPage.vue`（WebSocket） | — | — | — | 未测 | 三层图×3；私信投递/离线消息/媒体分享测试（待认领） |
...
| 维度 | 数量 | 明细 |
|---|---|---|
| 三层模型 | 24/39 | 具备：UC02/03/04/05/09/10/12/13；缺失：UC01/06/07/08/11 |
