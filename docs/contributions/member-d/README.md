...
- [`d-engagement.spec.ts`](../../../frontend/e2e/d-engagement.spec.ts)：Playwright 覆盖 UC05 视频互动持久化、UC09 预约闭环和 UC10 直播创建、互动、结束闭环；HTML 报告见 [`index.html`](../../testing/reports/engagement-e2e/index.html)。
...
- 开发基线：已同步 `origin/dev` 的 `ebf4e51`（文档重组冲突已解决，文档迁移至 `docs/contributions/member-d/` 新路径）。
