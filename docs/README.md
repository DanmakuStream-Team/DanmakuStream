...
| `deploy/` | CI/CD、镜像与部署运维说明 | [微服务 CI](deploy/microservice-ci.md) · [微服务 CD](deploy/microservice-cd.md) · [HPA/故障演练](deploy/hpa-chaos.md) |
...
4. HPA 清单、可重复压测、扩缩容曲线和 content 依赖故障脚本已建立；仍需在实际 k3s 运行并归档 Action URL/artifact，熔断实现和单体/微服务至少三轮性能对比仍未完成。
5. 单体 CD 与微服务 SHA 镜像推送/自动部署已建立；微服务 CD 会检查探针和版本、失败回滚并归档三服务日志，最终 tag 必须等所有实测证据齐全后由 `release-freeze` 创建。
